...
- 鍵盤的結構參考beekeeb及bastardkb鍵盤[Dilemma](https://docs.bastardkb.com/bg_dilemma/v2.html)設計，FFC連接座會固定在電路板背面，跟熱插拔座在同一面（請看下圖照片對比）。
...
觸控板上的電阻移除後，將觸控板本體及3片擋板組合在一起。首先，擋板`A`、`C`的洞口直徑不太一樣，視安裝的元件不同，擋板上下的順序也不同。
...
接著將軟排線接在觸控板上的FFC插槽內，並順著排線方向將擋板對好螺絲孔，用`6mm螺絲`固定起來。

![](pic/guide/l3.jpg)
![](pic/guide/l4.jpg)

最後依序將定位板、MCU擋板用`3mm螺絲`鎖上。
...
> RP2040 ProMicro的`通信問題`，請將電路板上的`P1`跳線移除後改至`P2`連接再做測試。
